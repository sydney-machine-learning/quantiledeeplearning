...
The next model to cover is Bi-Directional Long Short-Term Memory model (BDLSTM)
Feel free to change learning rate & batch size manually.

The only difference between uni/multi-variate is the data processing.
For the model, we just need to declare features = 5 instead of 1.
Because the input structure is different, output stays the same shape.
'''

print("\nCurrent Model: BDLSTM")
...
# Bi-directional Long-Short Term Memory Model (BD-LSTM) Multivariate
class BDLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size):
        ...
        self.lstms.append(nn.LSTM(input_size, hidden_neurons[0], 
        	batch_first=True, bidirectional=True))

        for i in range(1, num_layers):
            self.lstms.append(nn.LSTM(hidden_neurons[i-1]*2, hidden_neurons[i], 
            	batch_first=True, bidirectional=True))
        # * 2 because of bidirectional
        self.fc = nn.Linear(hidden_neurons[-1] * 2, output_size)  
...
for i in tqdm(range(num_exp)): 
	...
	model = BDLSTM(num_features, hidden_neurons, num_layers, output_size)
