...
The next model to cover is Bi-Directional Long Short-Term Memory model.
Note that learning rate & batch size are not customisable in the prompt
but they can be adjusted here.
'''

print("\nCurrent Model: BD-LSTM")
...
# DEFINITION: Bi-Directional Long-Short Term Memory model (BDLSTM)
# This defined structure is able to handle multiple hidden layers
class BDLSTM(nn.Module):
	'''
	number of features in a univariate time series = 1
	default neurons defined: [64, 32]
	number of layers is the length of neurons = 2
	output size is how many values we are predicting = 5
	'''
	def __init__(self, num_features, neurons, num_layers, output_size):

		super(BDLSTM, self).__init__() # initialise pytorch

		self.neurons = neurons
		self.num_layers = num_layers

		# this allows pytorch to track parameters for multiple lstm layers
		self.lstms = nn.ModuleList() 

		# define 1st lstm layer
		self.lstms.append(nn.LSTM(num_features, neurons[0], 
			batch_first = True, bidirectional = True)) 

		# subsequent lstm layer(s)
		for i in range(1, num_layers):
			# each layer has prev layer's size * 2 due to bi-directional nature
			self.lstms.append(nn.LSTM(neurons[i - 1] * 2, neurons[i],
				batch_first = True, bidirectional = True))

		# last layer (e.g. 32 * 2) -> output size (5)
		self.fc = nn.Linear(neurons[-1] * 2, output_size) 

	# forward pass - involve strict shape consistencies
	# allows the model to traverse into hidden layers 
	def forward(self, x): 
		for lstm_layer in self.lstms:
			x, _ = lstm_layer(x) 	# _ = hidden/cell states
		out = self.fc(x[:, -1, :]) 	# fc = fully connected, -1 = last time step
		return out
...
for i in tqdm(range(num_exp)): 
	...
	# use BDLSTM
	model = BDLSTM(num_features, hidden_neurons, hidden_layers, output_size)
	...
	for epoch in range(1, num_epochs + 1): # training loop
		model.train()
		for inputs, targets in df_train: # develop the model on the train set

			inputs = inputs.unsqueeze(-1) # add feature dimension to use model
			outputs = model(inputs) 
			loss = criterion(outputs, targets) # MSE (prev defined)
			optimizer.zero_grad() # backward and optimise - ADAM
...
print(f"\nAfter {num_exp} experimental runs, "
	f"here are the results for BDLSTM Model:")
